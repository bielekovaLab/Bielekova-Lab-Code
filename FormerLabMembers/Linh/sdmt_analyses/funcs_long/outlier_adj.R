outlier.adj <- function(input,option) {
  ...
  # identify the bland-altman thresholds
  ...
  # eliminate the guesses within each trial to keep only the first 
  # guess and the total score
  
    data <- input %>% group_by(PID,Date,Time) %>% 
      filter(Time.Vals == min(Time.Vals)) %>%
      ungroup()
    
    # filtering the data to only keep the score/sec difference which
    # are under the upper thresholds for their respective groups
